...
var createBox = function(startEndTensor) {
  return {
    startEndTensor,
...
var scaleBox = function(box, factors) {
  var starts = mul(box.startPoint, factors);
  var ends = mul(box.endPoint, factors);
  var newCoordinates = concat2d([starts, ends], 1);
...
function decodeBounds(boxOutputs, anchors, inputSize) {
  var boxStarts = slice(boxOutputs, [0, 1], [-1, 2]);
  var centers = add(boxStarts, anchors);
  var boxSizes = slice(boxOutputs, [0, 3], [-1, 2]);
  var boxSizesNormalized = div(boxSizes, inputSize);
  var centersNormalized = div(centers, inputSize);
  var halfBoxSize = div(boxSizesNormalized, 2);
  var starts = sub(centersNormalized, halfBoxSize);
  var ends = add(centersNormalized, halfBoxSize);
  var startNormalized = mul(starts, inputSize);
  var endNormalized = mul(ends, inputSize);
  var concatAxis = 1;
  return concat2d([startNormalized, endNormalized], concatAxis);
}
function getInputTensorDimensions(input) {
  return input instanceof Tensor ? [input.shape[0], input.shape[1]] : [input.height, input.width];
}
function flipFaceHorizontal(face, imageWidth) {
  ...
  if (face.topLeft instanceof Tensor && face.bottomRight instanceof Tensor) {
    var _a = tidy(function() {
      return [
        concat([
          slice(sub(imageWidth - 1, face.topLeft), 0, 1),
          slice(face.topLeft, 1, 1)
        ]),
        concat([
          sub(imageWidth - 1, slice(face.bottomRight, 0, 1)),
          slice(face.bottomRight, 1, 1)
        ])
      ];
    ...
    if (face.landmarks != null) {
      flippedLandmarks = tidy(function() {
        var a = sub(tensor1d([imageWidth - 1, 0]), face.landmarks);
        var b = tensor1d([1, -1]);
        var product = mul(a, b);
        return product;
      });
    ...
    if (face.landmarks != null) {
      flippedLandmarks = face.landmarks.map(function(coord) {
        return [
  ...
  if (face.probability != null) {
    flippedFace.probability = face.probability instanceof Tensor ? face.probability.clone() : face.probability;
  }
  return flippedFace;
}
function scaleBoxFromPrediction(face, scaleFactor) {
  return tidy(function() {
    ...
    return squeeze(scaleBox(box, scaleFactor).startEndTensor);
  });
}
var BlazeFaceModel = (
  /** @class */
  function() {
    function BlazeFaceModel2(model, width, height, maxFaces, iouThreshold, scoreThreshold) {
    ...
    BlazeFaceModel2.prototype.getBoundingBoxes = function(inputImage, returnTensors, annotateBoxes) {
      if (annotateBoxes === void 0) {
        annotateBoxes = true;
      }
      return __awaiter(this, void 0, void 0, function() {
        ...
        return __generator(this, function(_b) {
          switch (_b.label) {
            case 0:
              _a = tidy(function() {
                var resizedImage = image.resizeBilinear(inputImage, [_this.width, _this.height]);
                var normalizedImage = mul(sub(div(resizedImage, 255), 0.5), 2);
                var batchedPrediction = _this.blazeFaceModel.predict(normalizedImage);
                var prediction = squeeze(batchedPrediction);
                var decodedBounds = decodeBounds(prediction, _this.anchors, _this.inputSize);
                var logits = slice(prediction, [0, 0], [-1, 1]);
                var scores2 = squeeze(sigmoid(logits));
                return [prediction, decodedBounds, scores2];
              }), detectedOutputs = _a[0], boxes = _a[1], scores = _a[2];
              savedConsoleWarnFn = console.warn;
              console.warn = function() {
              };
              boxIndicesTensor = image.nonMaxSuppression(boxes, scores, this.maxFaces, this.iouThreshold, this.scoreThreshold);
              console.warn = savedConsoleWarnFn;
              return [4, boxIndicesTensor.array()];
            case 1:
              boxIndices = _b.sent();
              boxIndicesTensor.dispose();
              boundingBoxes = boxIndices.map(function(boxIndex) {
                return slice(boxes, [boxIndex, 0], [1, -1]);
              });
              if (!!returnTensors) return [3, 3];
              return [4, Promise.all(boundingBoxes.map(function(boundingBox) {
                return __awaiter(_this, void 0, void 0, function() {
                  var vals;
                  return __generator(this, function(_a2) {
                    switch (_a2.label) {
                      case 0:
                        return [4, boundingBox.array()];
                      case 1:
                        vals = _a2.sent();
                        boundingBox.dispose();
                        return [2, vals];
            ...
            case 3:
              ...
              annotatedBoxes = [];
              _loop_1 = function(i2) {
                var boundingBox = boundingBoxes[i2];
                var annotatedBox = tidy(function() {
                  var box = boundingBox instanceof Tensor ? createBox(boundingBox) : createBox(tensor2d(boundingBox));
                  if (!annotateBoxes) {
                    return box;
                  }
                  var boxIndex = boxIndices[i2];
                  var anchor;
                  if (returnTensors) {
                    anchor = slice(_this.anchors, [boxIndex, 0], [1, 2]);
                  } else {
                    anchor = _this.anchorsData[boxIndex];
                  }
                  var landmarks = reshape(squeeze(slice(detectedOutputs, [boxIndex, NUM_LANDMARKS - 1], [1, -1])), [NUM_LANDMARKS, -1]);
                  var probability = slice(scores, [boxIndex], [1]);
    ...
    BlazeFaceModel2.prototype.estimateFaces = function(input, returnTensors, flipHorizontal, annotateBoxes) {
      if (returnTensors === void 0) {
        returnTensors = false;
      }
      ...
      return __awaiter(this, void 0, void 0, function() {
        var _a, width, image2, _b, boxes, scaleFactor;
        var _this = this;
        return __generator(this, function(_c) {
          switch (_c.label) {
            case 0:
              _a = getInputTensorDimensions(input), width = _a[1];
              image2 = tidy(function() {
                if (!(input instanceof Tensor)) {
                  input = browser_exports.fromPixels(input);
                }
                return expandDims(cast(input, "float32"), 0);
              });
              return [4, this.getBoundingBoxes(image2, returnTensors, annotateBoxes)];
            case 1:
              ...
              if (returnTensors) {
                return [2, boxes.map(function(face) {
                  ...
                  var normalizedFace = {
                    topLeft: slice(scaledBox, [0], [2]),
                    bottomRight: slice(scaledBox, [2], [2])
                  };
                  if (annotateBoxes) {
                    var _a2 = face, landmarks = _a2.landmarks, probability = _a2.probability, anchor = _a2.anchor;
                    var normalizedLandmarks = mul(add(landmarks, anchor), scaleFactor);
              ...
              return [2, Promise.all(boxes.map(function(face) {
                return __awaiter(_this, void 0, void 0, function() {
                  var scaledBox, normalizedFace, boxData, _a2, landmarkData, boxData, probabilityData, anchor_1, _b2, scaleFactorX_1, scaleFactorY_1, scaledLandmarks;
                  var _this2 = this;
                  return __generator(this, function(_c2) {
                    switch (_c2.label) {
                      ...
                      case 1:
                        ...
                        return [3, 4];
                      case 2:
                        return [4, Promise.all([face.landmarks, scaledBox, face.probability].map(function(d) {
                      ...
                      case 3:
                        _a2 = _c2.sent(), landmarkData = _a2[0], boxData = _a2[1], probabilityData = _a2[2];
                        anchor_1 = face.anchor;
                        _b2 = scaleFactor, scaleFactorX_1 = _b2[0], scaleFactorY_1 = _b2[1];
                        scaledLandmarks = landmarkData.map(function(landmark) {
                          return [
...
function load(_a) {
  ...
  return __awaiter(this, void 0, void 0, function() {
    ...
    return __generator(this, function(_h) {
      switch (_h.label) {
        case 0:
          if (!(modelUrl != null)) return [3, 2];
          return [4, loadGraphModel(modelUrl)];
        case 1:
          blazeface = _h.sent();
          return [3, 4];
        case 2:
...
@tensorflow-models/blazeface/dist/blazeface.esm.js:
  (**
   * @license
  ...
  ***************************************************************************** *)
   ...
   *)
*/
//# sourceMappingURL=@tensorflow-models_blazeface.js.map
